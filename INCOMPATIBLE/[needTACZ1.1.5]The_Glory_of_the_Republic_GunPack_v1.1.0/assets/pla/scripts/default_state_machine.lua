...
local movement_track_states = {
    ...
    -- 行走, -1 是没有行走, 0 是在空中, 1 是正在瞄准, 2 是在向前走, 3 是向后退, 4 是向左走, 5是向右走
    walk = {
        mode = -1
    },
    -- 战术冲刺
...
-- 退出奔跑态
function movement_track_states.run.exit(this, context,input)
    ...
    context:runAnimation("run_1_hold", context:getTrack(BLENDING_TRACK_LINE, LOOP_TRACK) , true, PLAY_ONCE_STOP , 0.3)
    if context:isCrawl() then
        context:runAnimation("tactical_slide", context:findIdleTrack(BLENDING_TRACK_LINE, MOVEMENT_TRACK), true, PLAY_ONCE_HOLD, 0.3)
    end
end
...
function movement_track_states.run.update(this, context)
    ...
    if (context:isHolding(track)) then
        ...
        context:runAnimation("run_1", loop_track, true, LOOP, 0.2) -- 混合播放跑步平移动画
        -- 检测是否奔跑的标志位 0
        state.mode = 0
    ...
    if (state.mode ~= -1) then
        if (not context:isOnGround()) then
            ...
            if (state.mode ~= 1) then
                ...
                context:runAnimation("run_1_hold", loop_track, true, LOOP, 0.6)
            end
        else
            -- 如果玩家在地面，则切换回 run 动画
            if (state.mode ~= 0) then
                state.mode = 0
                context:runAnimation("run", track, true, LOOP, 0.2)
                context:runAnimation("run_1", loop_track, true, LOOP, 0.2)
            end
...
function movement_track_states.walk.update(this, context)
    ...
    elseif (context:isInputLeft()) then
        -- 如果正在向左面，则需要播放 walk_left 动画
        if (state.mode ~= 4) then
            state.mode = 4
            context:runAnimation("walk_left", track, true, LOOP, 0.4)
            context:anchorWalkDist() -- 打 walkDist 锚点，确保行走动画的起点一致
        end
    ...
    elseif (context:isInputRight()) then
        -- 如果正在向右面，则需要播放 walk_right 动画
        if (state.mode ~= 5) then
            state.mode = 5
            context:runAnimation("walk_right", track, true, LOOP, 0.4)
            context:anchorWalkDist() -- 打 walkDist 锚点，确保行走动画的起点一致
        end
